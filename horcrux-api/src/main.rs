...
use alerts::{Alert, AlertManager, AlertRule, NotificationChannel};
...
use backup::{Backup, BackupConfig, BackupJob, BackupManager, RetentionPolicy};
use cloudinit::{CloudInitConfig, CloudInitManager};
use cluster::{ArchitectureSummary, ClusterManager, Node};
use console::{ConsoleInfo, ConsoleManager, ConsoleType};
use firewall::{FirewallManager, FirewallRule, FirewallScope, SecurityGroup};
use horcrux_common::auth::{LoginRequest, LoginResponse};
use horcrux_common::VmConfig;
use monitoring::{MonitoringManager, NodeMetrics, ResourceMetrics, StorageMetrics};
use observability::{OtelConfig, OtelManager};
use secrets::VaultManager;
use serde::{Deserialize, Serialize};
use std::net::SocketAddr;
use std::sync::Arc;
use template::{CloneRequest, OsType, StorageType, Template, TemplateManager};
use tls::TlsManager;
...
async fn main() -> anyhow::Result<()> {
    ...
    match db::users::list_users(database.pool()).await {
        Ok(users) if users.is_empty() => {
            ...
            if admin_password == "admin" {
                ...
                tracing::warn!(
                    "⚠️  Please set ADMIN_PASSWORD environment variable for production!"
                );
            }

            let password_hash =
                hash_password(&admin_password).expect("Failed to hash admin password");
    ...
    info!(
        ...
        rate_limit_config.window.as_secs()
    );
    ...
    info!(
        "Auth rate limiter initialized: {} requests per {} seconds",
        auth_rate_limit_config.max_requests,
        auth_rate_limit_config.window.as_secs()
    );
    ...
    if horcrux_config.cni.enabled && horcrux_config.cni.bin_dir.exists() {
        ...
        tracing::warn!(
            "CNI binary directory not found at {:?} - CNI features disabled",
            horcrux_config.cni.bin_dir
        );
    }

    // Initialize Network Policy manager
    let network_policy_manager = Arc::new(tokio::sync::RwLock::new(
        sdn::policy::NetworkPolicyManager::new(),
    ));
    ...
    let snapshot_scheduler = Arc::new(vm::snapshot_scheduler::SnapshotScheduler::new(
    ...
    let state = Arc::new(AppState {
        ...
        audit_logger: Arc::new(AuditLogger::new(Some(
            horcrux_config.logging.audit_log.clone(),
        ))),
        database,
    ...
    let vm_getter = Arc::new(move |vm_id: &str| {
        ...
        Box::pin(async move { state.database.get_vm(&vm_id).await.ok() })
    ...
    let libvirt_manager = {
        ...
        match mgr.connect(None).await {
            ...
            Err(e) => {
                tracing::warn!(
                    "Failed to connect to libvirt: {} - VM metrics will use fallback",
                    e
                );
                None
            }
    ...
    let addr = format!(
        "{}:{}",
        horcrux_config.server.host, horcrux_config.server.port
    );
...
fn vm_routes() -> Router<Arc<AppState>> {
    Router::new()
        ...
        .route(
            "/api/snapshot-quotas/:id/usage",
            get(get_snapshot_quota_usage),
        )
        .route(
            "/api/snapshot-quotas/summary",
            get(get_snapshot_quota_summary),
        )
        .route(
            "/api/snapshot-quotas/:id/enforce",
            post(enforce_snapshot_quota),
        )
        ...
        .route(
            "/api/replication/jobs/:id/execute",
            post(execute_replication_job),
        )
        .route(
            "/api/replication/jobs/:id/status",
...
fn auth_protected_routes() -> Router<Arc<AppState>> {
    Router::new()
        ...
        .route(
            "/api/users/:username/api-keys/:key_id",
...
fn storage_routes() -> Router<Arc<AppState>> {
    Router::new()
        ...
        .route(
            "/api/gpu/devices/:pci_address/bind-vfio",
            post(bind_gpu_to_vfio),
        )
        .route(
            "/api/gpu/devices/:pci_address/unbind-vfio",
            post(unbind_gpu_from_vfio),
        )
        .route(
            "/api/gpu/devices/:pci_address/iommu-group",
            get(get_gpu_iommu_group),
        )
        ...
        .route(
            "/api/tls/certificate/generate",
            post(generate_self_signed_cert),
        )
        .route(
            "/api/tls/certificate/info/:path",
            get(get_certificate_info_endpoint),
        )
        // Vault endpoints
...
fn monitoring_routes() -> Router<Arc<AppState>> {
    Router::new()
        ...
        .route(
            "/api/alerts/:rule_id/:target/acknowledge",
        ...
        .route(
            "/api/observability/export/metrics",
            post(export_metrics_now),
        )
...
fn network_routes() -> Router<Arc<AppState>> {
    Router::new()
        ...
        .route(
            "/api/firewall/security-groups/:name",
            get(get_security_group),
        )
        ...
        .route(
            "/api/cni/attachments/:container_id",
            get(list_container_attachments),
        )
        .route(
            "/api/cni/capabilities/:plugin_type",
            get(get_cni_plugin_capabilities),
        )
        ...
        .route(
            "/api/network-policies/namespace/:namespace",
            get(list_policies_in_namespace),
        )
        .route(
            "/api/network-policies/:id/iptables",
            get(get_policy_iptables_rules),
        )
        .route(
            "/api/network-policies/:id/nftables",
            get(get_policy_nftables_rules),
        )
}
...
fn kubernetes_routes() -> Router<Arc<AppState>> {
    Router::new()
        ...
        .route(
            "/api/k8s/clusters/:cluster_id/reconnect",
            post(k8s_reconnect_cluster),
        )
        .route(
            "/api/k8s/clusters/:cluster_id/health",
            get(k8s_get_cluster_health),
        )
        .route(
            "/api/k8s/clusters/:cluster_id/version",
            get(k8s_get_cluster_version),
        )
        // Kubernetes namespaces
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces",
            get(k8s_list_namespaces),
        )
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces",
            post(k8s_create_namespace),
        )
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces/:namespace",
            get(k8s_get_namespace),
        )
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces/:namespace",
            delete(k8s_delete_namespace),
        )
        // Kubernetes nodes
        .route("/api/k8s/clusters/:cluster_id/nodes", get(k8s_list_nodes))
        .route(
            "/api/k8s/clusters/:cluster_id/nodes/:node",
            get(k8s_get_node),
        )
        .route(
            "/api/k8s/clusters/:cluster_id/nodes/:node/cordon",
            post(k8s_cordon_node),
        )
        .route(
            "/api/k8s/clusters/:cluster_id/nodes/:node/uncordon",
            post(k8s_uncordon_node),
        )
        .route(
            "/api/k8s/clusters/:cluster_id/nodes/:node/drain",
            post(k8s_drain_node),
        )
        // Kubernetes pods
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces/:namespace/pods",
            get(k8s_list_pods),
        )
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces/:namespace/pods/:pod",
            get(k8s_get_pod),
        )
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces/:namespace/pods/:pod",
            delete(k8s_delete_pod),
        )
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces/:namespace/pods/:pod/logs",
            get(k8s_get_pod_logs),
        )
        // Kubernetes deployments
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces/:namespace/deployments",
            get(k8s_list_deployments),
        )
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces/:namespace/deployments/:deployment",
            get(k8s_get_deployment),
        )
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces/:namespace/deployments/:deployment/scale",
            post(k8s_scale_deployment),
        )
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces/:namespace/deployments/:deployment/restart",
            post(k8s_restart_deployment),
        )
        // Kubernetes events
        .route("/api/k8s/clusters/:cluster_id/events", get(k8s_list_events))
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces/:namespace/events",
            get(k8s_list_namespace_events),
        )
        // Kubernetes services
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces/:namespace/services",
            get(k8s_list_services),
        )
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces/:namespace/services",
            post(k8s_create_service),
        )
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces/:namespace/services/:service",
            get(k8s_get_service),
        )
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces/:namespace/services/:service",
            delete(k8s_delete_service),
        )
        // Kubernetes ingresses
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces/:namespace/ingresses",
            get(k8s_list_ingresses),
        )
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces/:namespace/ingresses",
            post(k8s_create_ingress),
        )
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces/:namespace/ingresses/:ingress",
            get(k8s_get_ingress),
        )
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces/:namespace/ingresses/:ingress",
            delete(k8s_delete_ingress),
        )
        // Kubernetes configmaps
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces/:namespace/configmaps",
            get(k8s_list_configmaps),
        )
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces/:namespace/configmaps",
            post(k8s_create_configmap),
        )
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces/:namespace/configmaps/:name",
            get(k8s_get_configmap),
        )
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces/:namespace/configmaps/:name",
            delete(k8s_delete_configmap),
        )
        // Kubernetes secrets
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces/:namespace/secrets",
            get(k8s_list_secrets),
        )
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces/:namespace/secrets",
            post(k8s_create_secret),
        )
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces/:namespace/secrets/:name",
            get(k8s_get_secret),
        )
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces/:namespace/secrets/:name",
            delete(k8s_delete_secret),
        )
        // Kubernetes PVCs
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces/:namespace/pvcs",
            get(k8s_list_pvcs),
        )
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces/:namespace/pvcs",
        ...
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces/:namespace/pvcs/:name",
            get(k8s_get_pvc),
        )
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces/:namespace/pvcs/:name",
            delete(k8s_delete_pvc),
        )
        // Kubernetes statefulsets
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces/:namespace/statefulsets",
            get(k8s_list_statefulsets),
        )
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces/:namespace/statefulsets/:name",
            get(k8s_get_statefulset),
        )
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces/:namespace/statefulsets/:name/scale",
            post(k8s_scale_statefulset),
        )
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces/:namespace/statefulsets/:name",
            delete(k8s_delete_statefulset),
        )
        // Kubernetes daemonsets
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces/:namespace/daemonsets",
            get(k8s_list_daemonsets),
        )
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces/:namespace/daemonsets/:name",
            get(k8s_get_daemonset),
        )
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces/:namespace/daemonsets/:name",
            delete(k8s_delete_daemonset),
        )
        // Kubernetes jobs
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces/:namespace/jobs",
            get(k8s_list_jobs),
        )
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces/:namespace/jobs/:name",
            get(k8s_get_job),
        )
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces/:namespace/jobs/:name",
            delete(k8s_delete_job),
        )
        // Kubernetes cronjobs
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces/:namespace/cronjobs",
            get(k8s_list_cronjobs),
        )
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces/:namespace/cronjobs/:name",
            get(k8s_get_cronjob),
        )
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces/:namespace/cronjobs/:name",
            delete(k8s_delete_cronjob),
        )
        // Kubernetes metrics
        .route(
            "/api/k8s/clusters/:cluster_id/metrics/nodes",
            get(k8s_get_node_metrics),
        )
        .route(
            "/api/k8s/clusters/:cluster_id/namespaces/:namespace/metrics/pods",
            get(k8s_get_pod_metrics),
        )
        // Helm releases
        .route(
            "/api/k8s/clusters/:cluster_id/helm/releases",
            get(k8s_list_helm_releases),
        )
        .route(
            "/api/k8s/clusters/:cluster_id/helm/releases",
            post(k8s_install_helm_release),
        )
        .route(
            "/api/k8s/clusters/:cluster_id/helm/releases/:release_name",
            put(k8s_upgrade_helm_release),
        )
        .route(
            "/api/k8s/clusters/:cluster_id/helm/releases/:release_name",
            delete(k8s_uninstall_helm_release),
        )
        .route(
            "/api/k8s/clusters/:cluster_id/helm/releases/:release_name/rollback",
            post(k8s_rollback_helm_release),
        )
        .route(
            "/api/k8s/clusters/:cluster_id/helm/releases/:release_name/history",
            get(k8s_get_helm_release_history),
        )
        // Helm repos (global)
        .route("/api/k8s/helm/repos", get(k8s_list_helm_repos))
        .route("/api/k8s/helm/repos", post(k8s_add_helm_repo))
        .route(
            "/api/k8s/helm/repos/:repo_name",
            delete(k8s_remove_helm_repo),
        )
...
fn nas_routes() -> Router<Arc<AppState>> {
    Router::new()
        ...
        .route(
            "/api/nas/users/:user_id/password",
            post(nas_set_user_password),
        )
        ...
        .route(
            "/api/nas/groups/:group_id/members",
            get(nas_get_group_members),
        )
        .route(
            "/api/nas/groups/:group_id/members",
            post(nas_add_group_member),
        )
        .route(
            "/api/nas/groups/:group_id/members/:user_id",
            delete(nas_remove_group_member),
        )
        // NAS Pools
        ...
        .route(
            "/api/nas/datasets/:dataset_id/quota",
            post(nas_set_dataset_quota),
        )
        // NAS Snapshots
        .route(
            "/api/nas/datasets/:dataset_id/snapshots",
            get(nas_list_snapshots),
        )
        .route(
            "/api/nas/datasets/:dataset_id/snapshots",
            post(nas_create_snapshot),
        )
        .route("/api/nas/snapshots/:snapshot_id", get(nas_get_snapshot))
        .route(
            "/api/nas/snapshots/:snapshot_id",
            delete(nas_delete_snapshot),
        )
        .route(
            "/api/nas/snapshots/:snapshot_id/rollback",
            post(nas_rollback_snapshot),
        )
        .route(
            "/api/nas/snapshots/:snapshot_id/clone",
            post(nas_clone_snapshot),
        )
        // Snapshot Policies
        .route(
            "/api/nas/snapshot-policies",
            get(nas_list_snapshot_policies),
        )
        .route(
            "/api/nas/snapshot-policies",
            post(nas_create_snapshot_policy),
        )
        .route(
            "/api/nas/snapshot-policies/:policy_id",
            get(nas_get_snapshot_policy),
        )
        .route(
            "/api/nas/snapshot-policies/:policy_id",
            put(nas_update_snapshot_policy),
        )
        .route(
            "/api/nas/snapshot-policies/:policy_id",
            delete(nas_delete_snapshot_policy),
        )
        // NAS Replication
        .route(
            "/api/nas/replication/tasks",
            get(nas_list_replication_tasks),
        )
        .route(
            "/api/nas/replication/tasks",
            post(nas_create_replication_task),
        )
        .route(
            "/api/nas/replication/tasks/:task_id",
            get(nas_get_replication_task),
        )
        .route(
            "/api/nas/replication/tasks/:task_id",
            put(nas_update_replication_task),
        )
        .route(
            "/api/nas/replication/tasks/:task_id",
            delete(nas_delete_replication_task),
        )
        .route(
            "/api/nas/replication/tasks/:task_id/run",
            post(nas_run_replication_task),
        )
        .route(
            "/api/nas/replication/tasks/:task_id/history",
            get(nas_get_replication_history),
        )
        // NAS Services
        ...
        .route(
            "/api/nas/services/:service/restart",
            post(nas_restart_service),
        )
        ...
        .route(
            "/api/nas/smb/connections/:conn_id/disconnect",
            post(nas_disconnect_smb),
        )
        ...
        .route(
            "/api/nas/iscsi/targets/:target_id",
            get(nas_get_iscsi_target),
        )
        .route(
            "/api/nas/iscsi/targets/:target_id",
            put(nas_update_iscsi_target),
        )
        .route(
            "/api/nas/iscsi/targets/:target_id",
            delete(nas_delete_iscsi_target),
        )
        .route(
            "/api/nas/iscsi/targets/:target_id/luns",
            get(nas_list_iscsi_luns),
        )
        .route(
            "/api/nas/iscsi/targets/:target_id/luns",
            post(nas_create_iscsi_lun),
        )
        .route("/api/nas/iscsi/luns/:lun_id", delete(nas_delete_iscsi_lun))
        .route(
            "/api/nas/iscsi/targets/:target_id/acls",
            get(nas_list_iscsi_acls),
        )
        .route(
            "/api/nas/iscsi/targets/:target_id/acls",
            post(nas_create_iscsi_acl),
        )
        ...
        .route(
            "/api/nas/s3/buckets/:bucket_name",
            delete(nas_delete_s3_bucket),
        )
        .route("/api/nas/s3/access-keys", get(nas_list_s3_access_keys))
        .route("/api/nas/s3/access-keys", post(nas_create_s3_access_key))
        .route(
            "/api/nas/s3/access-keys/:key_id",
            delete(nas_delete_s3_access_key),
        )
        // Directory/Auth configuration
        .route("/api/nas/directory/config", get(nas_get_directory_config))
        .route(
            "/api/nas/directory/config",
            put(nas_update_directory_config),
        )
        // LDAP endpoints
        .route("/api/nas/directory/ldap/status", get(nas_get_ldap_status))
        .route(
            "/api/nas/directory/ldap/configure",
            post(nas_configure_ldap),
        )
        .route(
            "/api/nas/directory/ldap/test",
            post(nas_test_ldap_connection),
        )
        .route("/api/nas/directory/ldap/sync", post(nas_sync_ldap))
        .route("/api/nas/directory/ldap/users", get(nas_search_ldap_users))
        .route(
            "/api/nas/directory/ldap/groups",
            get(nas_search_ldap_groups),
        )
        // Kerberos endpoints
        .route(
            "/api/nas/directory/kerberos/status",
            get(nas_get_kerberos_status),
        )
        .route(
            "/api/nas/directory/kerberos/configure",
            post(nas_configure_kerberos),
        )
        .route(
            "/api/nas/directory/kerberos/kinit",
            post(nas_kerberos_kinit),
        )
        .route("/api/nas/directory/kerberos/klist", get(nas_kerberos_klist))
        .route(
            "/api/nas/directory/kerberos/kdestroy",
            post(nas_kerberos_kdestroy),
        )
        .route(
            "/api/nas/directory/kerberos/keytabs",
            get(nas_list_kerberos_keytabs),
        )
        .route(
            "/api/nas/directory/kerberos/keytab",
            post(nas_create_kerberos_keytab),
        )
        // Active Directory endpoints
        .route("/api/nas/directory/ad/status", get(nas_get_ad_status))
        .route(
            "/api/nas/directory/ad/join",
            post(nas_join_active_directory),
        )
        .route(
            "/api/nas/directory/ad/leave",
            post(nas_leave_active_directory),
        )
        .route("/api/nas/directory/ad/users", get(nas_list_ad_users))
        .route("/api/nas/directory/ad/groups", get(nas_list_ad_groups))
        .route(
            "/api/nas/directory/ad/user/:username/groups",
            get(nas_get_user_ad_groups),
        )
        .route("/api/nas/directory/ad/test-trust", post(nas_test_ad_trust))
        .route("/api/nas/directory/ad/ping-dc", post(nas_ping_ad_dc))
        .route(
            "/api/nas/directory/ad/verify-prereqs",
            post(nas_verify_ad_prereqs),
        )
        // Scheduler endpoints
        .route("/api/nas/scheduler/jobs", get(nas_list_scheduled_jobs))
        .route("/api/nas/scheduler/jobs", post(nas_create_scheduled_job))
        .route(
            "/api/nas/scheduler/jobs/:job_id",
            get(nas_get_scheduled_job),
        )
        .route(
            "/api/nas/scheduler/jobs/:job_id",
            put(nas_update_scheduled_job),
        )
        .route(
            "/api/nas/scheduler/jobs/:job_id",
            delete(nas_delete_scheduled_job),
        )
        .route(
            "/api/nas/scheduler/jobs/:job_id/run",
            post(nas_run_scheduled_job),
        )
        .route(
            "/api/nas/scheduler/jobs/:job_id/pause",
            post(nas_pause_scheduled_job),
        )
        .route(
            "/api/nas/scheduler/jobs/:job_id/resume",
            post(nas_resume_scheduled_job),
        )
        .route(
            "/api/nas/scheduler/jobs/:job_id/history",
            get(nas_get_job_history),
        )
        ...
        .route(
            "/api/nas/ftp/connections/:conn_id/disconnect",
            post(nas_disconnect_ftp),
        )
        ...
        .route(
            "/api/nas/webdav/connections",
            get(nas_list_webdav_connections),
        )
        ...
        .route(
            "/api/nas/afp/connections/:conn_id/disconnect",
            post(nas_disconnect_afp),
        )
        .route(
            "/api/nas/afp/timemachine",
            get(nas_list_timemachine_targets),
        )
}
...
mod nas_handlers {
    use super::*;
    use crate::nas::auth::{AuthManager, NasGroup, NasUser};
    ...
    pub async fn nas_list_shares(
        ...
        let json_shares: Vec<serde_json::Value> = shares
            .iter()
    ...
    pub async fn nas_get_share(
        ...
        match share {
            Some((id, name, path, owner, enabled, desc, created)) => Ok(Json(serde_json::json!({
                "id": id,
                "name": name,
                "path": path,
                "owner_user": owner,
    ...
    pub async fn nas_update_share(
        ...
        // Get existing share
        let existing =
            sqlx::query_as::<_, (String,)>("SELECT id FROM nas_shares WHERE id = ? OR name = ?")
                .bind(&id)
                .bind(&id)
                .fetch_optional(state.database.pool())
                .await
    ...
    pub async fn nas_enable_share(
        State(state): State<Arc<AppState>>,
        Path(id): Path<String>,
    ) -> Result<(), ApiError> {
        let now = chrono::Utc::now().timestamp();
        let result = sqlx::query(
            "UPDATE nas_shares SET enabled = 1, updated_at = ? WHERE id = ? OR name = ?",
        )
        .bind(now)
        .bind(&id)
        .bind(&id)
    ...
    pub async fn nas_disable_share(
        State(state): State<Arc<AppState>>,
        Path(id): Path<String>,
    ) -> Result<(), ApiError> {
        let now = chrono::Utc::now().timestamp();
        let result = sqlx::query(
            "UPDATE nas_shares SET enabled = 0, updated_at = ? WHERE id = ? OR name = ?",
        )
        .bind(now)
        .bind(&id)
        .bind(&id)
    ...
    pub async fn nas_update_user(
        ...
        let shell = body
            .get("shell")
            .and_then(|v| v.as_str())
            .unwrap_or(&existing.shell)
            .to_string();
        let home = body
            .get("home")
            .and_then(|v| v.as_str())
            .unwrap_or(&existing.home)
            .to_string();
        let full_name = body
            .get("full_name")
            .and_then(|v| v.as_str())
        ...
        let primary_group = manager
            .get_system_group_by_gid(existing.gid)
            .await
            .map(|g| g.name)
    ...
    pub async fn nas_delete_user(
        ...
        // Get group name from GID
        let primary_group = manager
            .get_system_group_by_gid(user_info.gid)
            .await
            .map(|g| g.name)
            .unwrap_or_else(|_| format!("{}", user_info.gid));

        let user = NasUser::new(id, user_info.username, user_info.uid, primary_group);
        manager
            .delete_system_user(&user)
            .await
            .map_err(|e| ApiError::Internal(format!("Failed to delete user: {}", e)))
    }
    pub async fn nas_set_user_password(
        State(_state): State<Arc<AppState>>,
        Path(id): Path<String>,
        Json(body): Json<serde_json::Value>,
    ) -> Result<(), ApiError> {
    ...
    pub async fn nas_create_group(
        ...
        let group = NasGroup::new(uuid::Uuid::new_v4().to_string(), name.to_string(), gid);
        manager
    ...
    pub async fn nas_update_group(
        ...
        Err(ApiError::Internal(
            "Group modification not supported".to_string(),
        ))
    }
    pub async fn nas_delete_group(
        State(_state): State<Arc<AppState>>,
        Path(id): Path<String>,
    ) -> Result<(), ApiError> {
        let manager = AuthManager::new();
        ...
        let group = NasGroup::new(id, group_info.name, group_info.gid);
        manager
            .delete_system_group(&group)
            .await
            .map_err(|e| ApiError::Internal(format!("Failed to delete group: {}", e)))
    }
    pub async fn nas_get_group_members(
        State(_state): State<Arc<AppState>>,
        Path(id): Path<String>,
    ) -> Result<Json<Vec<serde_json::Value>>, ApiError> {
        let manager = AuthManager::new();
        let group_info = manager
            ...
            .await
            .map_err(|_| ApiError::NotFound("Group not found".to_string()))?;
        let members: Vec<serde_json::Value> = group_info
            .members
            .iter()
    ...
    pub async fn nas_add_group_member(
        State(_state): State<Arc<AppState>>,
        Path(id): Path<String>,
        Json(body): Json<serde_json::Value>,
    ) -> Result<(), ApiError> {
        ...
        let manager = AuthManager::new();
        manager
            .add_user_to_group(username, &id)
            .await
            .map_err(|e| ApiError::Internal(format!("Failed to add user to group: {}", e)))
    }
    pub async fn nas_remove_group_member(
        State(_state): State<Arc<AppState>>,
    ...
    pub async fn nas_scrub_pool(
        ...
        Ok(Json(
            serde_json::json!({"status": "scrub started", "pool": id}),
        ))
    }
    #[cfg(not(feature = "nas-zfs"))]
    pub async fn nas_scrub_pool(
    ...
    #[cfg(feature = "nas-zfs")]
    pub async fn nas_get_pool_status(
        State(_state): State<Arc<AppState>>,
        Path(id): Path<String>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
        let manager = StorageManager::new();
    ...
    // Dataset handlers
    pub async fn nas_list_datasets(
        State(_state): State<Arc<AppState>>,
    ...
    pub async fn nas_create_dataset(
        State(_state): State<Arc<AppState>>,
        Path(pool_id): Path<String>,
        Json(body): Json<serde_json::Value>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
    ...
    pub async fn nas_update_dataset(
        State(_state): State<Arc<AppState>>,
        Path(_id): Path<String>,
        Json(_body): Json<serde_json::Value>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
        ...
        Err(ApiError::Internal(
            "Dataset property updates not yet implemented".to_string(),
        ))
    }
    pub async fn nas_delete_dataset(
        State(_state): State<Arc<AppState>>,
        Path(id): Path<String>,
    ) -> Result<(), ApiError> {
        use crate::nas::storage::datasets;
        datasets::destroy_dataset(&id)
            .await
            .map_err(|e| ApiError::Internal(format!("Failed to delete dataset: {}", e)))
    }
    pub async fn nas_set_dataset_quota(
        State(_state): State<Arc<AppState>>,
        Path(id): Path<String>,
        Json(body): Json<serde_json::Value>,
    ) -> Result<(), ApiError> {
        ...
        datasets::set_quota(&id, quota_bytes)
            .await
            .map_err(|e| ApiError::Internal(format!("Failed to set quota: {}", e)))
    }

    // Snapshot handlers
    pub async fn nas_list_snapshots(
        State(_state): State<Arc<AppState>>,
    ...
    pub async fn nas_create_snapshot(
        State(_state): State<Arc<AppState>>,
        Path(dataset_id): Path<String>,
        Json(body): Json<serde_json::Value>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
    ...
    pub async fn nas_get_snapshot(
        ...
        if parts.len() != 2 {
            return Err(ApiError::BadRequest(
                "Invalid snapshot format, expected 'dataset@snapshot'".to_string(),
            ));
        }
        let manager = StorageManager::new();
    ...
    pub async fn nas_delete_snapshot(
        State(_state): State<Arc<AppState>>,
        Path(id): Path<String>,
    ) -> Result<(), ApiError> {
    ...
    pub async fn nas_rollback_snapshot(
        State(_state): State<Arc<AppState>>,
        Path(id): Path<String>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
        use crate::nas::storage::snapshots;
        snapshots::rollback_snapshot(&id)
            .await
            .map_err(|e| ApiError::Internal(format!("Failed to rollback snapshot: {}", e)))?;
        Ok(Json(
            serde_json::json!({"status": "rolled back", "snapshot": id}),
        ))
    }
    pub async fn nas_clone_snapshot(
        State(_state): State<Arc<AppState>>,
        Path(id): Path<String>,
        Json(body): Json<serde_json::Value>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
        ...
        snapshots::clone_snapshot(&id, target)
            .await
            .map_err(|e| ApiError::Internal(format!("Failed to clone snapshot: {}", e)))?;
        Ok(Json(
            serde_json::json!({"status": "cloned", "source": id, "target": target}),
        ))
    }

    // Snapshot policy handlers
    pub async fn nas_list_snapshot_policies(
        State(state): State<Arc<AppState>>,
    ) -> Result<Json<Vec<serde_json::Value>>, ApiError> {
        ...
        let json_policies: Vec<serde_json::Value> = policies
            .iter()
            .map(
                |(id, name, dataset, schedule, hourly, daily, weekly, enabled, created)| {
                    serde_json::json!({
                        "id": id,
                        "name": name,
                        "dataset": dataset,
    ...
    pub async fn nas_create_snapshot_policy(
        State(state): State<Arc<AppState>>,
        Json(body): Json<serde_json::Value>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
        let name = body
    ...
    pub async fn nas_get_snapshot_policy(
        State(state): State<Arc<AppState>>,
        Path(id): Path<String>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
    ...
    pub async fn nas_update_snapshot_policy(
        State(state): State<Arc<AppState>>,
        Path(id): Path<String>,
        Json(body): Json<serde_json::Value>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
        ...
        let existing =
            sqlx::query_as::<_, (String,)>("SELECT id FROM nas_snapshot_policies WHERE id = ?")
                .bind(&id)
                .fetch_optional(state.database.pool())
                .await
                .map_err(|e| ApiError::Internal(format!("Database error: {}", e)))?;
        ...
        if let Some(schedule) = body.get("schedule").and_then(|v| v.as_str()) {
            sqlx::query(
                "UPDATE nas_snapshot_policies SET schedule = ?, updated_at = ? WHERE id = ?",
            )
            .bind(schedule)
            .bind(now)
            .bind(&id)
        ...
        if let Some(enabled) = body.get("enabled").and_then(|v| v.as_bool()) {
            sqlx::query(
                "UPDATE nas_snapshot_policies SET enabled = ?, updated_at = ? WHERE id = ?",
            )
            .bind(enabled)
            .bind(now)
            .bind(&id)
    ...
    pub async fn nas_delete_snapshot_policy(
        State(state): State<Arc<AppState>>,
        Path(id): Path<String>,
    ) -> Result<(), ApiError> {
    ...
    pub async fn nas_list_replication_tasks(
        State(state): State<Arc<AppState>>,
    ) -> Result<Json<Vec<serde_json::Value>>, ApiError> {
        ...
        let json_tasks: Vec<serde_json::Value> = tasks
            .iter()
            .map(
                |(id, name, source, host, target, schedule, enabled, last_run, created)| {
                    serde_json::json!({
                        "id": id,
                        "name": name,
    ...
    pub async fn nas_create_replication_task(
        State(state): State<Arc<AppState>>,
        Json(body): Json<serde_json::Value>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
        let name = body
    ...
    pub async fn nas_get_replication_task(
        State(state): State<Arc<AppState>>,
        Path(id): Path<String>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
    ...
    pub async fn nas_update_replication_task(
        State(state): State<Arc<AppState>>,
        Path(id): Path<String>,
        Json(body): Json<serde_json::Value>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
        ...
        let existing =
            sqlx::query_as::<_, (String,)>("SELECT id FROM nas_replication_tasks WHERE id = ?")
                .bind(&id)
                .fetch_optional(state.database.pool())
                .await
                .map_err(|e| ApiError::Internal(format!("Database error: {}", e)))?;
        ...
        if let Some(schedule) = body.get("schedule").and_then(|v| v.as_str()) {
            sqlx::query(
                "UPDATE nas_replication_tasks SET schedule = ?, updated_at = ? WHERE id = ?",
            )
            .bind(schedule)
            .bind(now)
            .bind(&id)
        ...
        if let Some(enabled) = body.get("enabled").and_then(|v| v.as_bool()) {
            sqlx::query(
                "UPDATE nas_replication_tasks SET enabled = ?, updated_at = ? WHERE id = ?",
            )
            .bind(enabled)
            .bind(now)
            .bind(&id)
    ...
    pub async fn nas_delete_replication_task(
        State(state): State<Arc<AppState>>,
        Path(id): Path<String>,
    ) -> Result<(), ApiError> {
    ...
    pub async fn nas_run_replication_task(
        State(state): State<Arc<AppState>>,
        Path(id): Path<String>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
    ...
    pub async fn nas_get_replication_history(
        State(state): State<Arc<AppState>>,
        Path(id): Path<String>,
    ) -> Result<Json<Vec<serde_json::Value>>, ApiError> {
    ...
    // Service handlers
    pub async fn nas_list_services(
        State(_state): State<Arc<AppState>>,
    ) -> Result<Json<Vec<serde_json::Value>>, ApiError> {
    ...
    pub async fn nas_get_service_status(
        State(_state): State<Arc<AppState>>,
    ...
    pub async fn nas_start_service(
        State(_state): State<Arc<AppState>>,
        Path(service): Path<String>,
    ) -> Result<(), ApiError> {
        let nas_service = parse_service_name(&service)?;
        manage_service(&nas_service, ServiceAction::Start)
            .await
            .map_err(|e| ApiError::Internal(format!("Failed to start service: {}", e)))
    }
    pub async fn nas_stop_service(
        State(_state): State<Arc<AppState>>,
        Path(service): Path<String>,
    ) -> Result<(), ApiError> {
        let nas_service = parse_service_name(&service)?;
        manage_service(&nas_service, ServiceAction::Stop)
            .await
            .map_err(|e| ApiError::Internal(format!("Failed to stop service: {}", e)))
    }
    pub async fn nas_restart_service(
        State(_state): State<Arc<AppState>>,
        Path(service): Path<String>,
    ) -> Result<(), ApiError> {
        let nas_service = parse_service_name(&service)?;
        manage_service(&nas_service, ServiceAction::Restart)
            .await
    ...
    // SMB handlers
    pub async fn nas_get_smb_config(
        State(_state): State<Arc<AppState>>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
        Ok(Json(serde_json::json!({
    ...
    #[cfg(feature = "smb")]
    pub async fn nas_update_smb_config(
        State(_state): State<Arc<AppState>>,
        Json(body): Json<serde_json::Value>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
        ...
        let config = SmbGlobalConfig {
            ...
            fruit_enabled: body
                ...
                .unwrap_or(true),
            spotlight_enabled: body
                .get("spotlight_enabled")
                .and_then(|v| v.as_bool())
                .unwrap_or(false),
            min_protocol: body
                .get("min_protocol")
    ...
    #[cfg(feature = "smb")]
    pub async fn nas_list_smb_connections(
        State(_state): State<Arc<AppState>>,
    ) -> Result<Json<Vec<serde_json::Value>>, ApiError> {
        let manager = SmbManager::new();
        match manager.get_connections().await {
            Ok(connections) => {
                let json_conns: Vec<serde_json::Value> = connections
                    .iter()
    ...
    #[cfg(feature = "smb")]
    pub async fn nas_disconnect_smb(
        State(_state): State<Arc<AppState>>,
        Path(conn_id): Path<String>,
    ) -> Result<(), ApiError> {
        // conn_id is the PID of the smbd process
        let pid: u32 = conn_id.parse().map_err(|_| {
            ApiError::BadRequest("Invalid connection ID (expected PID)".to_string())
        })?;

        let manager = SmbManager::new();
        manager
            .disconnect_session(pid)
            .await
    ...
    // NFS handlers
    pub async fn nas_get_nfs_config(
        State(_state): State<Arc<AppState>>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
        #[cfg(feature = "nfs-server")]
        {
    ...
    pub async fn nas_update_nfs_config(
        State(_state): State<Arc<AppState>>,
        Json(_body): Json<serde_json::Value>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
        #[cfg(feature = "nfs-server")]
        ...
        {
            Err(ApiError::Internal(
                "NFS server feature not enabled".to_string(),
            ))
        }
    }
    pub async fn nas_list_nfs_clients(
        State(_state): State<Arc<AppState>>,
    ) -> Result<Json<Vec<serde_json::Value>>, ApiError> {
        #[cfg(feature = "nfs-server")]
        {
    ...
    // iSCSI handlers
    #[cfg(feature = "iscsi-target")]
    pub async fn nas_list_iscsi_targets(
        State(_state): State<Arc<AppState>>,
    ) -> Result<Json<Vec<serde_json::Value>>, ApiError> {
        let manager = IscsiTargetManager::new();
        match manager.list_targets().await {
            Ok(targets) => {
                let json_targets: Vec<serde_json::Value> = targets
                    .iter()
    ...
    #[cfg(feature = "iscsi-target")]
    pub async fn nas_create_iscsi_target(
        State(_state): State<Arc<AppState>>,
        Json(body): Json<serde_json::Value>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
        use crate::nas::services::iscsi::{IscsiAcl, IscsiLun, IscsiTarget, LunType};
        ...
        let luns: Vec<IscsiLun> = if let Some(luns_arr) =
            body.get("luns").and_then(|v| v.as_array())
        {
            luns_arr
                .iter()
                .filter_map(|l| {
                    ...
                    let lun_type = if lun_type_str == "file" {
                        LunType::File
                    } else {
                        LunType::Block
                    };
        ...
        let acls: Vec<IscsiAcl> =
            if let Some(acls_arr) = body.get("acls").and_then(|v| v.as_array()) {
                acls_arr
                    .iter()
                    .filter_map(|a| {
    ...
    #[cfg(feature = "iscsi-target")]
    pub async fn nas_get_iscsi_target(
        State(_state): State<Arc<AppState>>,
        Path(id): Path<String>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
        let manager = IscsiTargetManager::new();
    ...
    #[cfg(feature = "iscsi-target")]
    pub async fn nas_update_iscsi_target(
        State(_state): State<Arc<AppState>>,
        Path(iqn): Path<String>,
        Json(body): Json<serde_json::Value>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
        use crate::nas::services::iscsi::{ChapAuth, IscsiAcl, IscsiLun, IscsiTarget, LunType};
    ...
    #[cfg(feature = "iscsi-target")]
    pub async fn nas_delete_iscsi_target(
        State(_state): State<Arc<AppState>>,
        Path(id): Path<String>,
    ) -> Result<(), ApiError> {
        let manager = IscsiTargetManager::new();
    ...
    #[cfg(feature = "iscsi-target")]
    pub async fn nas_list_iscsi_luns(
        State(_state): State<Arc<AppState>>,
        ...
        match manager.get_target(&target_id).await {
            Ok(target) => {
                let json_luns: Vec<serde_json::Value> = target
                    .luns
                    .iter()
    ...
    #[cfg(feature = "iscsi-target")]
    pub async fn nas_create_iscsi_lun(
        State(_state): State<Arc<AppState>>,
        Path(target_iqn): Path<String>,
        Json(body): Json<serde_json::Value>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
        ...
        let lun_id = body
            .get("lun_id")
            .and_then(|v| v.as_u64())
            .ok_or_else(|| ApiError::BadRequest("Missing 'lun_id' field".to_string()))?
            as u32;
    ...
    #[cfg(feature = "iscsi-target")]
    pub async fn nas_delete_iscsi_lun(
        State(_state): State<Arc<AppState>>,
    ...
    #[cfg(feature = "iscsi-target")]
    pub async fn nas_list_iscsi_acls(
        State(_state): State<Arc<AppState>>,
        Path(target_id): Path<String>,
    ) -> Result<Json<Vec<serde_json::Value>>, ApiError> {
    ...
    #[cfg(feature = "iscsi-target")]
    pub async fn nas_create_iscsi_acl(
        State(_state): State<Arc<AppState>>,
        Path(target_iqn): Path<String>,
        Json(body): Json<serde_json::Value>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
        let initiator_pattern = body
            .get("initiator_pattern")
            .and_then(|v| v.as_str())
            .or_else(|| body.get("initiator_address").and_then(|v| v.as_str()))
            .ok_or_else(|| {
                ApiError::BadRequest(
                    "Missing 'initiator_pattern' or 'initiator_address' field".to_string(),
                )
            })?;

        let manager = IscsiTargetManager::new();
        manager
            .set_acl(&target_iqn, initiator_pattern)
            .await
    ...
    #[cfg(feature = "iscsi-target")]
    pub async fn nas_delete_iscsi_acl(
        State(_state): State<Arc<AppState>>,
    ...
    // S3 handlers
    #[cfg(feature = "s3-gateway")]
    pub async fn nas_get_s3_status(
        State(_state): State<Arc<AppState>>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
    ...
    pub async fn nas_get_s3_status(
        ...
        Ok(Json(
            serde_json::json!({"running": false, "enabled": false}),
        ))
    }
    #[cfg(feature = "s3-gateway")]
    pub async fn nas_list_s3_buckets(
        State(_state): State<Arc<AppState>>,
    ) -> Result<Json<Vec<serde_json::Value>>, ApiError> {
        let manager = S3GatewayManager::new();
    ...
    #[cfg(feature = "s3-gateway")]
    pub async fn nas_create_s3_bucket(
        State(_state): State<Arc<AppState>>,
        Json(body): Json<serde_json::Value>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
        let name = body
    ...
    pub async fn nas_create_s3_bucket(
        ...
        Err(ApiError::Internal(
            "S3 gateway feature not enabled".to_string(),
        ))
    }
    #[cfg(feature = "s3-gateway")]
    pub async fn nas_get_s3_bucket(
        State(_state): State<Arc<AppState>>,
    ...
    #[cfg(feature = "s3-gateway")]
    pub async fn nas_delete_s3_bucket(
        State(_state): State<Arc<AppState>>,
        Path(name): Path<String>,
    ) -> Result<(), ApiError> {
        let manager = S3GatewayManager::new();
    ...
    #[cfg(feature = "s3-gateway")]
    pub async fn nas_create_s3_access_key(
        State(_state): State<Arc<AppState>>,
        Json(body): Json<serde_json::Value>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
        let user_id = body
    ...
    #[cfg(feature = "s3-gateway")]
    pub async fn nas_delete_s3_access_key(
        State(_state): State<Arc<AppState>>,
    ...
    pub async fn nas_get_directory_config(
        State(_state): State<Arc<AppState>>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
    ...
    pub async fn nas_update_directory_config(
        State(_state): State<Arc<AppState>>,
        Json(_body): Json<serde_json::Value>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
        ...
        {
            Err(ApiError::Internal(
                "No directory service features enabled".to_string(),
            ))
        }
    }
    pub async fn nas_test_ldap_connection(
        State(_state): State<Arc<AppState>>,
        Json(body): Json<serde_json::Value>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
        #[cfg(feature = "ldap-server")]
        {
            ...
            match output {
                ...
                _ => Ok(Json(
                    serde_json::json!({"success": false, "message": "Connection failed"}),
                )),
            }
        }
        #[cfg(not(feature = "ldap-server"))]
        {
            let _ = body;
            Err(ApiError::Internal(
                "LDAP server feature not enabled".to_string(),
            ))
        }
    }
    pub async fn nas_join_active_directory(
        State(_state): State<Arc<AppState>>,
        Json(body): Json<serde_json::Value>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
        #[cfg(feature = "ad")]
        ...
        #[cfg(not(feature = "ad"))]
        {
            let _ = body;
            Err(ApiError::Internal(
                "Active Directory integration not enabled".to_string(),
            ))
        }
    }
    pub async fn nas_leave_active_directory(
        State(_state): State<Arc<AppState>>,
    ) -> Result<(), ApiError> {
        #[cfg(feature = "ad")]
        {
            use tokio::process::Command;
            let output = Command::new("net").args(["ads", "leave"]).output().await;
            match output {
    ...
    pub async fn nas_get_ad_status(
        State(_state): State<Arc<AppState>>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
        #[cfg(feature = "ad")]
        {
            ...
            match output {
                Ok(out) if out.status.success() => {
                    // Get domain info
                    let info_output = Command::new("net").args(["ads", "info"]).output().await;
                    let domain = info_output
                        .ok()
    ...
    pub async fn nas_get_ldap_status(
        State(_state): State<Arc<AppState>>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
        #[cfg(feature = "ldap-server")]
        {
    ...
    pub async fn nas_configure_ldap(
        State(_state): State<Arc<AppState>>,
        Json(body): Json<serde_json::Value>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
        #[cfg(feature = "ldap-server")]
    ...
    pub async fn nas_search_ldap_users(
        State(_state): State<Arc<AppState>>,
    ...
    pub async fn nas_search_ldap_groups(
        State(_state): State<Arc<AppState>>,
        Query(params): Query<std::collections::HashMap<String, String>>,
    ) -> Result<Json<Vec<serde_json::Value>>, ApiError> {
        #[cfg(feature = "ldap-server")]
    ...
    pub async fn nas_get_kerberos_status(
        State(_state): State<Arc<AppState>>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
    ...
    pub async fn nas_configure_kerberos(
        State(_state): State<Arc<AppState>>,
        Json(body): Json<serde_json::Value>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
        #[cfg(feature = "kerberos")]
        ...
        #[cfg(not(feature = "kerberos"))]
        {
            let _ = body;
            Err(ApiError::Internal(
                "Kerberos feature not enabled".to_string(),
            ))
        }
    }

    pub async fn nas_kerberos_kinit(
        State(_state): State<Arc<AppState>>,
        Json(body): Json<serde_json::Value>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
        #[cfg(feature = "kerberos")]
        {
            ...
            let output = if let Some(kt) = keytab {
                ...
                return Err(ApiError::BadRequest(
                    "Keytab required for non-interactive kinit".to_string(),
                ));
            };

            match output {
                Ok(out) if out.status.success() => Ok(Json(
                    serde_json::json!({"status": "success", "principal": principal}),
                )),
                Ok(out) => {
                    let stderr = String::from_utf8_lossy(&out.stderr);
    ...
    pub async fn nas_kerberos_klist(
        State(_state): State<Arc<AppState>>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
        #[cfg(feature = "kerberos")]
        {
    ...
    pub async fn nas_kerberos_kdestroy(
        State(_state): State<Arc<AppState>>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
        #[cfg(feature = "kerberos")]
        {
    ...
    pub async fn nas_list_kerberos_keytabs(
        State(_state): State<Arc<AppState>>,
        Query(params): Query<std::collections::HashMap<String, String>>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
        #[cfg(feature = "kerberos")]
        {
            ...
            let output = Command::new("klist").args(["-k", &keytab]).output().await;
            match output {
                Ok(out) if out.status.success() => {
                    let stdout = String::from_utf8_lossy(&out.stdout);
                    Ok(Json(serde_json::json!({
    ...
    pub async fn nas_create_kerberos_keytab(
        State(_state): State<Arc<AppState>>,
        Json(body): Json<serde_json::Value>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
        #[cfg(feature = "kerberos")]
    ...
    pub async fn nas_list_ad_users(
        State(_state): State<Arc<AppState>>,
    ) -> Result<Json<Vec<serde_json::Value>>, ApiError> {
        #[cfg(feature = "ad")]
        {
            use tokio::process::Command;
            let output = Command::new("wbinfo").args(["-u"]).output().await;
            match output {
                Ok(out) if out.status.success() => {
                    let stdout = String::from_utf8_lossy(&out.stdout);
        ...
        {
            Err(ApiError::Internal(
                "Active Directory feature not enabled".to_string(),
            ))
        }
    }

    pub async fn nas_list_ad_groups(
        State(_state): State<Arc<AppState>>,
    ) -> Result<Json<Vec<serde_json::Value>>, ApiError> {
        #[cfg(feature = "ad")]
        {
            use tokio::process::Command;
            let output = Command::new("wbinfo").args(["-g"]).output().await;
            match output {
                Ok(out) if out.status.success() => {
                    let stdout = String::from_utf8_lossy(&out.stdout);
    ...
    pub async fn nas_get_user_ad_groups(
        State(_state): State<Arc<AppState>>,
        ...
        {
            use tokio::process::Command;
            let output = Command::new("id").args([&username]).output().await;
            match output {
                Ok(out) if out.status.success() => {
                    let stdout = String::from_utf8_lossy(&out.stdout);
    ...
    pub async fn nas_test_ad_trust(
        State(_state): State<Arc<AppState>>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
        #[cfg(feature = "ad")]
        {
    ...
    pub async fn nas_ping_ad_dc(
        State(_state): State<Arc<AppState>>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
        #[cfg(feature = "ad")]
        {
            use tokio::process::Command;
            let output = Command::new("wbinfo").args(["--ping-dc"]).output().await;
            match output {
                Ok(out) if out.status.success() => {
                    let stdout = String::from_utf8_lossy(&out.stdout);
                    Ok(Json(serde_json::json!({
    ...
    pub async fn nas_verify_ad_prereqs(
        State(_state): State<Arc<AppState>>,
        Json(body): Json<serde_json::Value>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
        #[cfg(feature = "ad")]
        {
            ...
            let dns_check = Command::new("host").args([domain]).output().await;
            let dns_ok = dns_check.map(|o| o.status.success()).unwrap_or(false);

            // Check if required packages are installed
            let samba_check = Command::new("which").args(["net"]).output().await;
    ...
    pub async fn nas_list_scheduled_jobs(
        State(state): State<Arc<AppState>>,
    ) -> Result<Json<Vec<serde_json::Value>>, ApiError> {
        ...
        let json_jobs: Vec<serde_json::Value> = jobs
            .iter()
            .map(
                |(id, name, job_type, schedule, enabled, last_run, next_run, created)| {
                    serde_json::json!({
                        "id": id,
                        "name": name,
                        "job_type": job_type,
    ...
    pub async fn nas_create_scheduled_job(
        State(state): State<Arc<AppState>>,
        Json(body): Json<serde_json::Value>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
        use crate::nas::scheduler::{CronSchedule, JobType, ScheduledJob};
        ...
        let job_type = match job_type_str.to_lowercase().as_str() {
            ...
            "retention" | "retention_cleanup" => {
                let dataset = body
                    ...
                    .and_then(|v| v.as_str())
                    .ok_or_else(|| {
                        ApiError::BadRequest("Retention job requires 'dataset' field".to_string())
                    })?;
                let keep_count = body
                    .get("keep_count")
                    .and_then(|v| v.as_u64())
    ...
    pub async fn nas_get_scheduled_job(
        State(state): State<Arc<AppState>>,
        Path(id): Path<String>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
    ...
    pub async fn nas_update_scheduled_job(
        State(state): State<Arc<AppState>>,
        Path(id): Path<String>,
        Json(body): Json<serde_json::Value>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
        ...
        // Verify job exists
        let existing =
            sqlx::query_as::<_, (String,)>("SELECT id FROM nas_scheduled_jobs WHERE id = ?")
                .bind(&id)
                .fetch_optional(state.database.pool())
                .await
                .map_err(|e| ApiError::Internal(format!("Database error: {}", e)))?;
    ...
    pub async fn nas_delete_scheduled_job(
        State(state): State<Arc<AppState>>,
        Path(id): Path<String>,
    ) -> Result<(), ApiError> {
    ...
    pub async fn nas_run_scheduled_job(
        State(state): State<Arc<AppState>>,
        Path(id): Path<String>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
        // Get job details
    ...
    pub async fn nas_pause_scheduled_job(
        State(state): State<Arc<AppState>>,
        Path(id): Path<String>,
    ) -> Result<(), ApiError> {
        let now = chrono::Utc::now().timestamp();
        let result =
            sqlx::query("UPDATE nas_scheduled_jobs SET enabled = 0, updated_at = ? WHERE id = ?")
                .bind(now)
                .bind(&id)
                .execute(state.database.pool())
                .await
    ...
    pub async fn nas_resume_scheduled_job(
        State(state): State<Arc<AppState>>,
        Path(id): Path<String>,
    ) -> Result<(), ApiError> {
        let now = chrono::Utc::now().timestamp();
        let result =
            sqlx::query("UPDATE nas_scheduled_jobs SET enabled = 1, updated_at = ? WHERE id = ?")
                .bind(now)
                .bind(&id)
                .execute(state.database.pool())
                .await
    ...
    pub async fn nas_get_job_history(
        State(state): State<Arc<AppState>>,
        Path(id): Path<String>,
    ) -> Result<Json<Vec<serde_json::Value>>, ApiError> {
    ...
    pub async fn nas_get_scheduler_status(
        State(state): State<Arc<AppState>>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
        ...
        let enabled_jobs: i64 =
            sqlx::query_scalar("SELECT COUNT(*) FROM nas_scheduled_jobs WHERE enabled = 1")
                .fetch_one(state.database.pool())
                .await
                .unwrap_or(0);
        let running_jobs: i64 =
            sqlx::query_scalar("SELECT COUNT(*) FROM nas_job_history WHERE status = 'running'")
                .fetch_one(state.database.pool())
                .await
                .unwrap_or(0);
    ...
    pub async fn nas_get_ftp_config(
        State(_state): State<Arc<AppState>>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
        #[cfg(feature = "ftp")]
        {
    ...
    #[cfg(feature = "ftp")]
    pub async fn nas_update_ftp_config(
        State(_state): State<Arc<AppState>>,
        Json(body): Json<serde_json::Value>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
        use crate::nas::shares::ftp::{FtpGlobalConfig, FtpManager};
        use std::collections::HashMap;
        ...
        let config = FtpGlobalConfig {
            server_name: body
                .get("server_name")
                .and_then(|v| v.as_str())
                .unwrap_or("Horcrux FTP Server")
                .to_string(),
            port: body.get("port").and_then(|v| v.as_u64()).unwrap_or(21) as u16,
            passive_port_range: body
                .get("passive_port_range")
                .and_then(|v| v.as_str())
                .map(|s| {
                    let parts: Vec<&str> = s.split('-').collect();
                    if parts.len() == 2 {
                        (
                            parts[0].parse().unwrap_or(49152),
                            parts[1].parse().unwrap_or(65534),
                        )
                    } else {
                        (49152, 65534)
                    }
            ...
            max_clients_per_host: body
                .get("max_clients_per_host")
                .and_then(|v| v.as_u64())
                .unwrap_or(10) as u32,
            timeout_idle: body
                .get("timeout_idle")
            ...
            timeout_no_transfer: body
                .get("timeout_no_transfer")
                .and_then(|v| v.as_u64())
                .unwrap_or(900) as u32,
            allow_anonymous: body
                .get("allow_anonymous")
                .and_then(|v| v.as_bool())
                .unwrap_or(false),
            anonymous_root: body
            ...
            tls_required: body
                ...
                .and_then(|v| v.as_bool())
                .unwrap_or(false),
            chroot_local_users: body
                .get("chroot_local_users")
                .and_then(|v| v.as_bool())
                .unwrap_or(true),
            require_valid_shell: body
                .get("require_valid_shell")
                .and_then(|v| v.as_bool())
                .unwrap_or(true),
            use_sendfile: body
                .get("use_sendfile")
    ...
    #[cfg(feature = "ftp")]
    pub async fn nas_list_ftp_connections(
        State(_state): State<Arc<AppState>>,
    ) -> Result<Json<Vec<serde_json::Value>>, ApiError> {
        use crate::nas::shares::ftp::FtpManager;
        let manager = FtpManager::new();
    ...
    #[cfg(feature = "ftp")]
    pub async fn nas_disconnect_ftp(
        State(_state): State<Arc<AppState>>,
        Path(conn_id): Path<String>,
    ) -> Result<(), ApiError> {
        use crate::nas::shares::ftp::FtpManager;
    ...
    pub async fn nas_get_webdav_config(
        State(_state): State<Arc<AppState>>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
        #[cfg(feature = "webdav")]
        {
    ...
    #[cfg(feature = "webdav")]
    pub async fn nas_update_webdav_config(
        State(_state): State<Arc<AppState>>,
        Json(body): Json<serde_json::Value>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
        use crate::nas::shares::webdav::{WebdavGlobalConfig, WebdavManager};
        ...
        let config = WebdavGlobalConfig {
            ...
            ssl_certificate: body
                .get("ssl_certificate")
                .and_then(|v| v.as_str())
                .map(|s| s.to_string()),
            ssl_certificate_key: body
            ...
            client_body_temp_path: body
                .get("client_body_temp_path")
                .and_then(|v| v.as_str())
                .unwrap_or("/var/lib/nginx/webdav")
                .to_string(),
    ...
    pub async fn nas_list_webdav_connections(
        State(_state): State<Arc<AppState>>,
    ) -> Result<Json<Vec<serde_json::Value>>, ApiError> {
    ...
    pub async fn nas_get_afp_config(
        State(_state): State<Arc<AppState>>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
        #[cfg(feature = "afp")]
        {
    ...
    #[cfg(feature = "afp")]
    pub async fn nas_update_afp_config(
        State(_state): State<Arc<AppState>>,
        Json(body): Json<serde_json::Value>,
    ) -> Result<Json<serde_json::Value>, ApiError> {
        use crate::nas::shares::afp::{AfpGlobalConfig, AfpManager};
        use std::collections::HashMap;
    ...
    #[cfg(feature = "afp")]
    pub async fn nas_list_afp_connections(
        State(_state): State<Arc<AppState>>,
    ) -> Result<Json<Vec<serde_json::Value>>, ApiError> {
        use crate::nas::shares::afp::AfpManager;
        let manager = AfpManager::new();
    ...
    #[cfg(feature = "afp")]
    pub async fn nas_disconnect_afp(
        State(_state): State<Arc<AppState>>,
        Path(conn_id): Path<String>,
    ) -> Result<(), ApiError> {
        use crate::nas::shares::afp::AfpManager;
    ...
    #[cfg(feature = "afp")]
    pub async fn nas_list_timemachine_targets(
        State(_state): State<Arc<AppState>>,
    ) -> Result<Json<Vec<serde_json::Value>>, ApiError> {
        use crate::nas::shares::afp::AfpManager;
        let manager = AfpManager::new();
        ...
        let tm_targets: Vec<serde_json::Value> = shares
            .iter()
...
async fn health_detailed(State(state): State<Arc<AppState>>) -> Json<health::HealthResponse> {
...
async fn create_vm(
    ...
    state
        .ws_state
        .broadcast_vm_created(vm.id.clone(), vm_name, username);
...
async fn start_vm(
    ...
    state
        .ws_state
        .broadcast_vm_status(id.clone(), old_status, format!("{:?}", vm.status));
...
async fn delete_vm(
    ...
    let username = auth_user
        .map(|u| u.username.clone())
        .unwrap_or_else(|| "system".to_string());
    state.ws_state.broadcast_vm_deleted(id, vm_name, username);
...
async fn restore_backup(
    ...
    state
        .backup_manager
        .restore_backup(&id, payload.target_id)
        .await?;
    Ok(StatusCode::OK)
}
...
async fn clone_vm_cross_node(
    ...
    let manager =
        CrossNodeCloneManager::new(state.config.paths.vm_storage.to_string_lossy().to_string());
...
async fn create_snapshot_quota(
    ...
    use vm::snapshot_quota::{CleanupPolicy, QuotaType};
...
async fn enforce_snapshot_quota(
    ...
    let freed_bytes = state
        .snapshot_quota_manager
        .enforce_quota(&id, req.snapshot_ids)
        .await?;
...
async fn query_audit_events(
    ...
    let event_type = params.event_type.and_then(|s| match s.as_str() {
    ...
    let severity = params.severity.and_then(|s| match s.as_str() {
...
async fn get_failed_logins(
    ...
    let events = state
        .audit_logger
        .get_failed_logins(params.user, limit)
        .await;
    Ok(Json(events))
}
...
async fn detect_brute_force_attempts(
    ...
    let suspects = state
        .audit_logger
        .detect_brute_force(threshold, window_minutes)
        .await;
    Ok(Json(suspects))
}
...
async fn list_containers(
    State(state): State<Arc<AppState>>,
) -> Json<Vec<horcrux_common::ContainerConfig>> {
...
async fn create_container(
    ...
    state
        .ws_state
        .broadcast(websocket::WsEvent::ContainerStatusChanged {
...
async fn delete_container(
    ...
    state
        .ws_state
        .broadcast(websocket::WsEvent::ContainerDeleted {
...
async fn clone_container(
    ...
    let cloned_container = state
        .container_manager
        .clone_container(&source_id, &req.target_id, &req.target_name, req.snapshot)
        .await?;
    ...
    info!(
        "Container {} cloned to {} successfully",
...
async fn apply_retention(
    ...
    state
        .backup_manager
        .apply_retention(&target_id, &policy)
        .await?;
    Ok(StatusCode::OK)
}
...
async fn list_users(State(state): State<Arc<AppState>>) -> Json<Vec<horcrux_common::auth::User>> {
...
async fn list_roles(State(state): State<Arc<AppState>>) -> Json<Vec<horcrux_common::auth::Role>> {
...
async fn list_firewall_rules(State(state): State<Arc<AppState>>) -> Json<Vec<FirewallRule>> {
    let rules = state
        .firewall_manager
        .list_rules(FirewallScope::Datacenter)
        .await;
    Json(rules)
}
...
async fn add_firewall_rule(
    ...
    state
        .firewall_manager
        .add_rule(FirewallScope::Datacenter, rule)
        .await?;
    Ok(StatusCode::CREATED)
}
...
async fn delete_firewall_rule(
    ...
    state
        .firewall_manager
        .delete_rule(FirewallScope::Datacenter, &id)
        .await?;
    Ok(StatusCode::NO_CONTENT)
}

async fn list_security_groups(State(state): State<Arc<AppState>>) -> Json<Vec<SecurityGroup>> {
...
async fn get_node_stats(State(state): State<Arc<AppState>>) -> Json<Option<NodeMetrics>> {
    let metrics = state.monitoring_manager.get_node_metrics().await;
    Json(metrics)
}

async fn get_all_vm_stats(State(state): State<Arc<AppState>>) -> Json<Vec<ResourceMetrics>> {
...
async fn get_all_container_stats(State(state): State<Arc<AppState>>) -> Json<Vec<ResourceMetrics>> {
...
async fn get_all_storage_stats(State(state): State<Arc<AppState>>) -> Json<Vec<StorageMetrics>> {
...
async fn list_cluster_nodes(State(state): State<Arc<AppState>>) -> Json<Vec<Node>> {
...
async fn get_cluster_architecture(State(state): State<Arc<AppState>>) -> Json<ArchitectureSummary> {
...
async fn list_alert_rules(State(state): State<Arc<AppState>>) -> Json<Vec<AlertRule>> {
...
async fn list_active_alerts(State(state): State<Arc<AppState>>) -> Json<Vec<Alert>> {
...
async fn acknowledge_alert(
    ...
    state
        .alert_manager
        .acknowledge_alert(&rule_id, &target, &req.user)
        .await?;
    Ok(StatusCode::OK)
}
...
async fn get_otel_config(State(state): State<Arc<AppState>>) -> Result<Json<OtelConfig>, ApiError> {
...
async fn export_metrics_now(State(state): State<Arc<AppState>>) -> Result<StatusCode, ApiError> {
...
async fn list_certificates(State(state): State<Arc<AppState>>) -> Json<Vec<tls::CertificateInfo>> {
...
async fn get_vault_config(State(state): State<Arc<AppState>>) -> Json<secrets::VaultConfig> {
...
async fn list_storage_pools(State(state): State<Arc<AppState>>) -> Json<Vec<storage::StoragePool>> {
...
async fn list_ha_resources(State(state): State<Arc<AppState>>) -> Json<Vec<HaResourceResponse>> {
...
async fn get_ha_status(State(state): State<Arc<AppState>>) -> Json<HaStatusResponse> {
    let resources = state.ha_manager.list_resources().await;
...
async fn list_ha_groups(State(state): State<Arc<AppState>>) -> Json<Vec<ha::HaGroup>> {
...
async fn migrate_vm(
    ...
    info!(
        "Started migration of VM {} to node {}, job ID: {}",
        vm_id, req.target_node, job_id
    );

    // Broadcast migration started event
    state
        .ws_state
        .broadcast_migration_started(vm_id.clone(), source_node, req.target_node.clone());
...
async fn test_webhook(
    ...
    state
        .webhook_manager
        .trigger_event(
            webhooks::WebhookEventType::Custom("test".to_string()),
            test_data,
        )
        .await
        .map_err(|e| ApiError::Internal(e.to_string()))?;
...
async fn create_cni_network(
    ...
    let config = sdn::cni::CniConfig {
        ...
        ipam: sdn::cni::IpamConfig {
            ...
            range_end: None,
            gateway: req.gateway,
            routes: vec![sdn::cni::RouteConfig {
...
async fn get_cni_plugin_capabilities(
    ...
    let plugin_type: sdn::cni::CniPluginType =
        serde_json::from_str(&format!("\"{}\"", plugin_type))
            .map_err(|e| ApiError::BadRequest(format!("Invalid plugin type: {}", e)))?;

    let manager = state.cni_manager.read().await;
    let capabilities = manager
        .get_capabilities(&plugin_type)
        .await
        .map_err(|e| ApiError::Internal(e.to_string()))?;
...
mod k8s_handlers {
    ...
    pub async fn list_clusters(State(state): State<Arc<AppState>>) -> Json<Vec<K8sCluster>> {
    ...
    pub async fn get_cluster_version(
        ...
        let version = state
            .kubernetes_manager
            .get_cluster_version(&cluster_id)
            .await?;
        Ok(Json(version))
    }
    ...
    pub async fn list_namespaces(
        ...
        let namespaces =
            crate::kubernetes::cluster_resources::namespaces::list_namespaces(&client).await?;
        Ok(Json(namespaces))
    }
    ...
    pub async fn create_namespace(
        ...
        let client = state.kubernetes_manager.get_client(&cluster_id).await?;
        let ns =
            crate::kubernetes::cluster_resources::namespaces::create_namespace(&client, &payload)
                .await?;
    ...
    pub async fn get_namespace(
        ...
        let client = state.kubernetes_manager.get_client(&cluster_id).await?;
        let ns =
            crate::kubernetes::cluster_resources::namespaces::get_namespace(&client, &namespace)
                .await?;
        Ok(Json(ns))
    }
    ...
    pub async fn delete_namespace(
        ...
        let client = state.kubernetes_manager.get_client(&cluster_id).await?;
        crate::kubernetes::cluster_resources::namespaces::delete_namespace(&client, &namespace)
            .await?;
        Ok(StatusCode::NO_CONTENT)
    }
    ...
    pub async fn get_node(
        ...
        let client = state.kubernetes_manager.get_client(&cluster_id).await?;
        let node_info =
            crate::kubernetes::cluster_resources::nodes::get_node(&client, &node).await?;
        Ok(Json(node_info))
    }
    ...
    pub async fn get_pod(
        ...
        let client = state.kubernetes_manager.get_client(&cluster_id).await?;
        let pod_info =
            crate::kubernetes::workloads::pods::get_pod(&client, &namespace, &pod).await?;
        Ok(Json(pod_info))
    }
    ...
    pub async fn list_deployments(
        ...
        let client = state.kubernetes_manager.get_client(&cluster_id).await?;
        let deployments =
            crate::kubernetes::workloads::deployments::list_deployments(&client, &namespace)
                .await?;
        Ok(Json(deployments))
    }
    ...
    pub async fn get_deployment(
        ...
        let client = state.kubernetes_manager.get_client(&cluster_id).await?;
        let deploy = crate::kubernetes::workloads::deployments::get_deployment(
            &client,
            &namespace,
            &deployment,
        )
    ...
    pub async fn restart_deployment(
        ...
        let client = state.kubernetes_manager.get_client(&cluster_id).await?;
        crate::kubernetes::workloads::deployments::restart_deployment(
            &client,
            &namespace,
            &deployment,
        )
    ...
    pub async fn list_namespace_events(
        ...
        let client = state.kubernetes_manager.get_client(&cluster_id).await?;
        let events =
            crate::kubernetes::observability::events::list_events(&client, Some(&namespace))
                .await?;
        Ok(Json(events))
    }
    ...
    pub async fn list_services(
        ...
        let client = state.kubernetes_manager.get_client(&cluster_id).await?;
        let services =
            crate::kubernetes::networking::services::list_services(&client, &namespace).await?;
        Ok(Json(services))
    }
    ...
    pub async fn get_service(
        ...
        let client = state.kubernetes_manager.get_client(&cluster_id).await?;
        let svc =
            crate::kubernetes::networking::services::get_service(&client, &namespace, &service)
                .await?;
        Ok(Json(svc))
    }
    ...
    pub async fn create_service(
        ...
        let client = state.kubernetes_manager.get_client(&cluster_id).await?;
        let svc =
            crate::kubernetes::networking::services::create_service(&client, &payload).await?;
    ...
    pub async fn delete_service(
        ...
        let client = state.kubernetes_manager.get_client(&cluster_id).await?;
        crate::kubernetes::networking::services::delete_service(&client, &namespace, &service)
            .await?;
        Ok(StatusCode::NO_CONTENT)
    }
    ...
    pub async fn list_ingresses(
        ...
        let client = state.kubernetes_manager.get_client(&cluster_id).await?;
        let ingresses =
            crate::kubernetes::networking::ingress::list_ingresses(&client, &namespace).await?;
        Ok(Json(ingresses))
    }
    ...
    pub async fn get_ingress(
        ...
        let client = state.kubernetes_manager.get_client(&cluster_id).await?;
        let ing =
            crate::kubernetes::networking::ingress::get_ingress(&client, &namespace, &ingress)
                .await?;
        Ok(Json(ing))
    }
    ...
    pub async fn delete_ingress(
        ...
        let client = state.kubernetes_manager.get_client(&cluster_id).await?;
        crate::kubernetes::networking::ingress::delete_ingress(&client, &namespace, &ingress)
            .await?;
        Ok(StatusCode::NO_CONTENT)
    }
    ...
    pub async fn list_configmaps(
        ...
        let client = state.kubernetes_manager.get_client(&cluster_id).await?;
        let cms =
            crate::kubernetes::config_storage::configmaps::list_configmaps(&client, &namespace)
                .await?;
        Ok(Json(cms))
    }
    ...
    pub async fn get_configmap(
        ...
        let client = state.kubernetes_manager.get_client(&cluster_id).await?;
        let cm = crate::kubernetes::config_storage::configmaps::get_configmap(
    ...
    pub async fn create_configmap(
        ...
        let client = state.kubernetes_manager.get_client(&cluster_id).await?;
        let cm = crate::kubernetes::config_storage::configmaps::create_configmap(&client, &payload)
            .await?;
    ...
    pub async fn delete_configmap(
        ...
        let client = state.kubernetes_manager.get_client(&cluster_id).await?;
        crate::kubernetes::config_storage::configmaps::delete_configmap(&client, &namespace, &name)
            .await?;
        Ok(StatusCode::NO_CONTENT)
    }
    ...
    pub async fn list_secrets(
        ...
        let client = state.kubernetes_manager.get_client(&cluster_id).await?;
        let secrets =
            crate::kubernetes::config_storage::secrets::list_secrets(&client, &namespace).await?;
        Ok(Json(secrets))
    }
    ...
    pub async fn get_secret(
        ...
        let client = state.kubernetes_manager.get_client(&cluster_id).await?;
        let secret =
            crate::kubernetes::config_storage::secrets::get_secret(&client, &namespace, &name)
                .await?;
        Ok(Json(secret))
    }
    ...
    pub async fn create_secret(
        ...
        let client = state.kubernetes_manager.get_client(&cluster_id).await?;
        let secret =
            crate::kubernetes::config_storage::secrets::create_secret(&client, &payload).await?;
    ...
    pub async fn delete_secret(
        ...
        let client = state.kubernetes_manager.get_client(&cluster_id).await?;
        crate::kubernetes::config_storage::secrets::delete_secret(&client, &namespace, &name)
            .await?;
        Ok(StatusCode::NO_CONTENT)
    }
    ...
    pub async fn get_pvc(
        ...
        let client = state.kubernetes_manager.get_client(&cluster_id).await?;
        let pvc =
            crate::kubernetes::config_storage::pvcs::get_pvc(&client, &namespace, &name).await?;
        Ok(Json(pvc))
    }
    ...
    pub async fn list_statefulsets(
        ...
        let client = state.kubernetes_manager.get_client(&cluster_id).await?;
        let sts =
            crate::kubernetes::workloads::statefulsets::list_statefulsets(&client, &namespace)
                .await?;
        Ok(Json(sts))
    }
    ...
    pub async fn get_statefulset(
        ...
        let client = state.kubernetes_manager.get_client(&cluster_id).await?;
        let sts =
            crate::kubernetes::workloads::statefulsets::get_statefulset(&client, &namespace, &name)
                .await?;
        Ok(Json(sts))
    }
    ...
    pub async fn delete_statefulset(
        ...
        let client = state.kubernetes_manager.get_client(&cluster_id).await?;
        crate::kubernetes::workloads::statefulsets::delete_statefulset(&client, &namespace, &name)
            .await?;
        Ok(StatusCode::NO_CONTENT)
    }
    ...
    pub async fn list_daemonsets(
        ...
        let client = state.kubernetes_manager.get_client(&cluster_id).await?;
        let ds =
            crate::kubernetes::workloads::daemonsets::list_daemonsets(&client, &namespace).await?;
        Ok(Json(ds))
    }
    ...
    pub async fn get_daemonset(
        ...
        let client = state.kubernetes_manager.get_client(&cluster_id).await?;
        let ds =
            crate::kubernetes::workloads::daemonsets::get_daemonset(&client, &namespace, &name)
                .await?;
        Ok(Json(ds))
    }
    ...
    pub async fn delete_daemonset(
        ...
        let client = state.kubernetes_manager.get_client(&cluster_id).await?;
        crate::kubernetes::workloads::daemonsets::delete_daemonset(&client, &namespace, &name)
            .await?;
        Ok(StatusCode::NO_CONTENT)
    }
    ...
    pub async fn list_cronjobs(
        ...
        let client = state.kubernetes_manager.get_client(&cluster_id).await?;
        let cronjobs =
            crate::kubernetes::workloads::jobs::list_cronjobs(&client, &namespace).await?;
        Ok(Json(cronjobs))
    }
    ...
    pub async fn get_cronjob(
        ...
        let client = state.kubernetes_manager.get_client(&cluster_id).await?;
        let cronjob =
            crate::kubernetes::workloads::jobs::get_cronjob(&client, &namespace, &name).await?;
        Ok(Json(cronjob))
    }
    ...
    pub async fn get_pod_metrics(
        ...
        let client = state.kubernetes_manager.get_client(&cluster_id).await?;
        let metrics =
            crate::kubernetes::observability::metrics::get_pod_metrics(&client, &namespace).await?;
        Ok(Json(metrics))
    }
    ...
    pub async fn install_helm_release(
        ...
        let kubeconfig_path = format!("/tmp/kubeconfig-{}", cluster_id);
        let release =
            crate::kubernetes::helm::releases::install_release(&kubeconfig_path, &payload).await?;
    ...
    pub async fn upgrade_helm_release(
        ...
        let _client = state.kubernetes_manager.get_client(&cluster_id).await?;
        let kubeconfig_path = format!("/tmp/kubeconfig-{}", cluster_id);
        let release = crate::kubernetes::helm::releases::upgrade_release(
            &kubeconfig_path,
            &release_name,
            &payload,
        )
    ...
    pub async fn uninstall_helm_release(
        ...
        crate::kubernetes::helm::releases::uninstall_release(
            &kubeconfig_path,
            &release_name,
            namespace,
        )
    ...
    pub async fn rollback_helm_release(
        ...
        let kubeconfig_path = format!("/tmp/kubeconfig-{}", cluster_id);
        let namespace = query.namespace.as_deref().unwrap_or("default");
        crate::kubernetes::helm::releases::rollback_release(
            &kubeconfig_path,
            &release_name,
            namespace,
            payload.revision,
    ...
    pub async fn get_helm_release_history(
        ...
        let kubeconfig_path = format!("/tmp/kubeconfig-{}", cluster_id);
        let namespace = query.namespace.as_deref().unwrap_or("default");
        let history = crate::kubernetes::helm::releases::get_release_history(
            &kubeconfig_path,
            &release_name,
            namespace,
        )
    ...
    pub async fn search_helm_charts(
        ...
        let charts = crate::kubernetes::helm::repos::search_charts(
            &query.keyword,
            query.all_versions.unwrap_or(false),
        )
        .await?;
        Ok(Json(charts))
    }
