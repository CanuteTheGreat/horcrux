...
impl WebDavManager {
    ...
    pub async fn add_share(&self, share: &NasShare) -> Result<()> {
        ...
        tokio::fs::write(&conf_path, config)
            .await
            .map_err(|e| Error::Internal(format!("Failed to write WebDAV config: {}", e)))?;
    ...
    pub async fn add_share_extended(
        ...
        if !Path::new(&enabled_path).exists() {
            tokio::fs::symlink(&conf_path, &enabled_path)
                .await
                .map_err(|e| Error::Internal(format!("Failed to enable site: {}", e)))?;
        }
    ...
    pub async fn enable_share(&self, share_id: &str) -> Result<()> {
        ...
        if !Path::new(&enabled_path).exists() {
            tokio::fs::symlink(&conf_path, &enabled_path)
                .await
                .map_err(|e| Error::Internal(format!("Failed to enable share: {}", e)))?;
        }
    ...
    pub async fn disable_share(&self, share_id: &str) -> Result<()> {
        ...
        if Path::new(&enabled_path).exists() {
            tokio::fs::remove_file(&enabled_path)
                .await
                .map_err(|e| Error::Internal(format!("Failed to disable share: {}", e)))?;
        }
...
"#,
    ...
    fn generate_extended_nginx_config(
        ...
        server_block.push_str(&format!("# Share: {} ({})\n", share.name, share.id));
        ...
        if config.ssl.enabled {
            server_block.push_str(&format!("    listen {} ssl http2;\n", config.listen_port));
            server_block.push_str(&format!(
                "    listen [::]{} ssl http2;\n",
                config.listen_port
            ));
        ...
        if config.access_log {
            ...
            server_block.push_str(&format!("    access_log {} combined;\n", log_path));
        } else {
        ...
        // WebDAV location
        server_block.push_str(&format!("    location /webdav/{} {{\n", share.id));
        ...
        if let Some(ref dav_ext) = config.dav_extensions {
            if dav_ext.caldav_enabled {
                server_block.push_str(&format!("\n    location {} {{\n", dav_ext.caldav_path));
                server_block.push_str(&format!("        alias {}/calendars;\n", share.path));
                server_block.push_str("        dav_methods PUT DELETE MKCOL COPY MOVE;\n");
                server_block
                    .push_str("        dav_ext_methods PROPFIND OPTIONS LOCK UNLOCK REPORT;\n");
                server_block.push_str("        dav_access user:rw group:rw all:r;\n");
                server_block.push_str("        create_full_put_path on;\n");
            ...
            if dav_ext.carddav_enabled {
                ...
                server_block.push_str(&format!("        alias {}/contacts;\n", share.path));
                server_block.push_str("        dav_methods PUT DELETE MKCOL COPY MOVE;\n");
                server_block
                    .push_str("        dav_ext_methods PROPFIND OPTIONS LOCK UNLOCK REPORT;\n");
                server_block.push_str("        dav_access user:rw group:rw all:r;\n");
    ...
    pub async fn write_full_config(&self, shares: &[NasShare]) -> Result<()> {
        ...
        let mut entries = tokio::fs::read_dir(&self.nginx_conf_dir)
            .await
            .map_err(|e| Error::Internal(format!("Failed to read nginx conf dir: {}", e)))?;
    ...
    async fn reload_nginx(&self) -> Result<()> {
        ...
        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            return Err(Error::Internal(format!("nginx reload failed: {}", stderr)));
        }
    ...
    pub async fn add_user(&self, share_id: &str, username: &str, password: &str) -> Result<()> {
    ...
    pub async fn list_users(&self, share_id: &str) -> Result<Vec<WebDavUser>> {
        ...
        let content = tokio::fs::read_to_string(&htpasswd_path)
            .await
            .map_err(|e| Error::Internal(format!("Failed to read htpasswd file: {}", e)))?;
    ...
    pub async fn setup_caldav_carddav(&self, share_path: &str) -> Result<()> {
        ...
        tokio::fs::create_dir_all(&calendars_path)
            .await
            .map_err(|e| Error::Internal(format!("Failed to create calendars directory: {}", e)))?;

        tokio::fs::create_dir_all(&contacts_path)
            .await
            .map_err(|e| Error::Internal(format!("Failed to create contacts directory: {}", e)))?;
