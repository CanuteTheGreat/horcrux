...
pub fn MetricsExplorerPage() -> impl IntoView {
    ...
    // Build PromQL query from builder components - defined early for use in actions
    ...
    // Parse time range string to start/end times - defined early for use in actions
    let parse_time_range = move |range: &str| -> (chrono::DateTime<chrono::Utc>, chrono::DateTime<chrono::Utc>) {
    ...
    // Save query to history helper - stub that doesn't actually save for now
    let save_query_to_history = move |_query: String, _time_range: String| {
    ...
    let execute_query = create_action(move |_: &()| async move {
        ...
        match query_metrics(query_request).await {
            Ok(results) => {
                ...
                save_query_to_history(query, time_range.get());
            }
                                                ...
                                                if let Some(window) = web_sys::window() {
                                                    if let Some(document) = window.document() {
                                                        if let (Some(key_element), Some(value_element)) = (
                                                            ...
                                                            if let (Ok(key_input), Ok(value_input)) = (
                                                                key_element.dyn_into::<web_sys::HtmlInputElement>(),
                                                                value_element.dyn_into::<web_sys::HtmlInputElement>()
                                                            ) {
                ...
                view! {
                    <div class="query-results">
                        ...
                        {move || if loading.get() {
                            ...
                            let results_for_table = results.clone();
                            let results_for_chart = results.clone();
                            let results_for_summary = results.clone();
                            view! {
                                <div class="results-content">
                                    {move || if chart_type.get() == "table" || show_raw_data.get() {
                                        let table_data = results_for_table.clone();
                                        view! {
                                            <div class="results-table">
                                                <table class="data-table">
                                                    ...
                                                    <tbody>
                                                        {table_data.iter().flat_map(|result| &result.values).cloned().collect::<Vec<_>>().into_iter().map(|point| {
                                        ...
                                        let chart_data = results_for_chart.clone();
                                        view! {
                                            <div class="chart-container">
                                                <div class="chart-placeholder" id="metrics-chart">
                                                    "Chart will be rendered here - "{chart_type.get()}" chart for "{chart_data.len()}" series"
                                                </div>
                                            </div>
                                        }.into_view()
                                    }}
                                    ...
                                    <div class="results-summary">
                                        <div class="summary-stats">
                                            <span class="stat">"Series: "{results_for_summary.len()}</span>
                                            <span class="stat">"Data Points: "{results_for_summary.iter().map(|r| r.values.len()).sum::<usize>()}</span>
