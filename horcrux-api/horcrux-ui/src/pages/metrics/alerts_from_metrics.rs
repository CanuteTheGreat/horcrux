...
pub fn AlertsFromMetricsPage() -> impl IntoView {
    ...
    let save_alert_action = create_action(move |_: &()| async move {
        ...
        let result = if edit_rule_id.get().is_some() {
            api::update_alert_rule(rule).await
        } else {
            api::create_alert_rule(rule).await
        };
    ...
    let edit_rule = move |rule: AlertRule| {
        ...
        set_alert_description.set(rule.description.unwrap_or_default());
        set_alert_query.set(rule.metric);
        set_alert_condition.set(rule.condition);
        set_alert_threshold.set(rule.threshold);
        set_alert_duration.set(format!("{}s", rule.duration_seconds));
                                        ...
                                        <div class="rule-details">
                                            ...
                                            {rule.last_triggered.map(|triggered| view! {
                                                <div class="detail-item">
                                                    ...
                                                    <span class="detail-value">{triggered.to_rfc3339()}</span>
                                                </div>
                                            })}
                                            <div class="detail-item">
                                                <span class="detail-label">Created:</span>
                                                <span class="detail-value">{rule.created_at.to_rfc3339()}</span>
                                            </div>
                                        </div>
                                    </div>
                                }
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="channels-list">
                                            {move || notification_channels.get().into_iter().map(|channel| {
                                                let channel_id_for_check = channel.id.clone();
                                                let channel_id_for_change = channel.id.clone();
                                                let is_selected = move || selected_channels.get().contains(&channel_id_for_check);
                                                view! {
                                                    <label class="channel-checkbox">
                                                        <input
                                                            ...
                                                            on:change=move |ev| {
                                                                ...
                                                                let cid = channel_id_for_change.clone();
                                                                if event_target_checked(&ev) {
                                                                    if !channels.contains(&cid) {
                                                                        channels.push(cid);
                                                                    }
                                                                } else {
                                                                    channels.retain(|id| id != &channel_id_for_change);
                                                                }
