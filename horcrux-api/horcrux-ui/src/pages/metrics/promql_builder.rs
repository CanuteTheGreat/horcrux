...
pub fn PromQLBuilderPage() -> impl IntoView {
    ...
    let validate_query_local = {
        ...
        move |query: &str| {
            ...
            set_query_validation.set(Some(result));
        }
    };
    ...
    let rebuild_query = {
        ...
        move || {
            ...
            for (i, component) in components.iter().enumerate() {
                ...
                // Convert component to PromQL string based on its type
                        ...
                        view! {
                            <div class="panel functions-panel">
                                ...
                                <div class="functions-list">
                                    {
                                        let functions_by_category = promql_functions.clone().into_iter()
                                            ...
                                            on:click=move |_| {
                                                if let Some(window) = web_sys::window() {
                                                    ...
                                                    spawn_local(async move {
                                                        let _ = clipboard.write_text(&query);
                                                    });
                                                }
                                            }
                                        >
                                        ...
                                        {if !validation.suggestions.is_empty() {
                                            view! {
                                                <div class="validation-suggestions">
                                                    <h5>"Suggestions:"</h5>
                                                    <ul>
                                                        {validation.suggestions.iter().cloned().map(|suggestion| view! {
                                                            <li>{suggestion}</li>
                                                        }).collect::<Vec<_>>()}
                                                    </ul>
                                                </div>
