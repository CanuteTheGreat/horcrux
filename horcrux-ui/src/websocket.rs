...
pub struct WsClient {
    url: String,
    ws: Option<WebSocket>,
    on_message: WriteSignal<Option<WsEvent>>,
...
impl WsClient {
    ...
    pub fn new(url: String) -> (Self, ReadSignal<Option<WsEvent>>, ReadSignal<Option<String>>, ReadSignal<bool>) {
        let (on_message, set_on_message) = create_signal(None);
        let (on_error, set_on_error) = create_signal(None);
        let (connected, set_connected) = create_signal(false);

        let client = Self {
            url,
            ws: None,
            on_message: set_on_message,
            on_error: set_on_error,
            connected: set_connected,
        };

        (client, on_message, on_error, connected)
    }

    /// Connect to WebSocket server
    pub fn connect(&mut self, topics: Vec<String>) -> Result<(), JsValue> {
        ...
        let onopen_callback = Closure::wrap(Box::new(move |_| {
            ...
            let subscription = WsSubscription { topics: topics.clone() };
        ...
        let onerror_callback = Closure::wrap(Box::new(move |e: ErrorEvent| {
            ...
            on_error.set(Some(format!("WebSocket error: {:?}", e.message())));
            connected.set(false);
...
pub fn use_websocket(topics: Vec<String>) -> (ReadSignal<Option<WsEvent>>, ReadSignal<bool>, ReadSignal<Option<String>>) {
    let (mut client, events, errors, connected) = WsClient::new("ws://localhost:8006/api/ws".to_string());

    // Connect on mount
    create_effect(move |_| {
        let _ = client.connect(topics.clone());
    });
