...
pub async fn handle_replication_command(
    ...
    match command {
        ReplicationCommands::List => {
            ...
            let rows: Vec<ReplicationJobRow> =
                jobs.into_iter().map(ReplicationJobRow::from).collect();
        ...
        ReplicationCommands::Execute { id } => {
            ...
            let response: ExecuteResponse = api
                .post(
                    &format!("/api/replication/jobs/{}/execute", id),
                    &serde_json::json!({}),
                )
                .await?;
            ...
            output::print_info(&format!(
                "Transferred: {}",
                format_bytes(response.bytes_transferred)
            ));
        }
        ...
        ReplicationCommands::Delete { id } => {
            ...
            let confirm = Confirm::new()
                .with_prompt(format!(
                    "Are you sure you want to delete replication job {}?",
                    id
                ))
                .interact()?;
